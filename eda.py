"""
Tashkent Air Quality — Exploratory Data Analysis
=================================================
Run:  python eda.py
Output: plots saved to output/, summary printed to console.
"""
...
print(f"\n{df[['pm25','pm10','no2','o3','aqi']].describe().round(1)}")
...
bars = ax.bar(monthly_avg["month_name"], monthly_avg["pm25"],
              color=sns.color_palette("RdYlGn_r", 12))
ax.axhline(15, color="green",  linestyle="--", linewidth=1.2, label="WHO guideline (15 µg/m³)")
ax.axhline(35, color="orange", linestyle="--", linewidth=1.2, label="US EPA Moderate threshold")
...
ax.set_title("Average PM2.5 by Hour of Day — Tashkent", fontweight="bold")
ax.set_xlabel("Hour")
ax.set_ylabel("PM2.5 (µg/m³)")
...
ax.set_title("Pollutant Correlation Matrix — Tashkent", fontweight="bold")
plt.tight_layout()
...
# ── 8. Key numpy stats ─────────────────────────────────────────────────────────
pm = df["pm25"].values
print("\n--- PM2.5 Statistics (numpy) ---")
print(f"  Mean:        {np.mean(pm):.1f} µg/m³")
print(f"  Median:      {np.median(pm):.1f} µg/m³")
print(f"  Std dev:     {np.std(pm):.1f}")
print(f"  95th pct:    {np.percentile(pm, 95):.1f} µg/m³")
print(f"  99th pct:    {np.percentile(pm, 99):.1f} µg/m³")
print(f"  Above WHO:   {(pm > 15).mean()*100:.1f}% of readings")
print(f"  Above EPA:   {(pm > 35).mean()*100:.1f}% of readings")
